...
CREATE TABLE mining_history (
    ...
    CONSTRAINT CHK_mining_history_type CHECK (type IN ('BROADCAST_PROGRESS', 'BROADCAST_WATCH', 'REFERRAL_REWARD'))
);
...
COMMENT ON COLUMN mining_history.type IS '채굴 유형 (BROADCAST_PROGRESS: 방송진행, BROADCAST_WATCH: 방송시청, REFERRAL_REWARD: 레퍼럴 리워드)';
...
CREATE INDEX IDX_mining_history_efficiency ON mining_history(efficiency);
